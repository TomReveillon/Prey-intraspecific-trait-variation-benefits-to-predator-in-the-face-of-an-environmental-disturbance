...
Conv=as.numeric(subset(DataC2, Strain=="CR6")[,3])

# Parameters
a=0.031         # Predator attack rate on species
h=29.07         # Predator handling time on species
...
ggplot(DataTO, aes(Comp1, Ing1)) +
  ...
  ylab(expression('A'~'ingestion probability difference'~'('*p[Delta]*')')) + 
  xlab(expression('A'[1]~'half-saturation constant'~'('*K[1]*')')) +
...
Plot1ODE1[Plot1ODE1$Organism=="A1",]$Density=log(MeltODE1[MeltODE1$Organism=="A1",]$Density+1)/5
...
tiff('Panel Population Dynamics No Evolution 1.tiff', units="in", width=20, height=20, res=1000)
ggplot(Plot1ODE1, aes(Time, Density, group=Organism)) +
  geom_line(aes(color=Organism), size=0.5) +
  ylab(expression('Ln predator density'~'('*rotifers~mL^-1*')')) + xlab(expression('Time (days)')) +
  ...
  sec.axis=sec_axis(~.*20/4, expression('Ln prey density'~'('*cells~mL^-1*')'), labels=function(y) sprintf("%.0f", y), breaks=seq(0,20,by=5))) +
  scale_x_continuous(labels=function(y) sprintf("%.0f", y), breaks=seq(0,200,by=100), limits=c(0,200)) +
  theme(panel.background=element_blank(), panel.border=element_blank()) +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank()) +
  theme(axis.line=element_line(colour="black", linetype="solid", size=0.5)) +
  scale_color_manual(values=c("A1"="royalblue2","P"="firebrick3")) +
...
tiff('Panel Population Dynamics No Evolution 2.tiff', units="in", width=20, height=20, res=1000)
ggplot(Plot2ODE1, aes(Time, Density, group=Organism)) +
  ...
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank()) +
  theme(axis.line=element_line(colour="black", linetype="solid", size=0.5)) +
  scale_color_manual(values=c("A"="royalblue2","P"="firebrick3")) +
  theme(strip.background=element_blank(), strip.text.x=element_blank()) +
  annotate("segment", x=-Inf, xend=Inf, y=-Inf, yend=-Inf, size=1.2) +
  annotate("segment", x=-Inf, xend=-Inf, y=-Inf, yend=Inf, size=1.2) +
  annotate("segment", x=Inf, xend=Inf, y=-Inf, yend=Inf, size=1.4) +
...
SaveODE1$P0=as.character(1-as.numeric(SaveODE1$P1))
...
Predator1=ggplot(Data2, aes(B, P0, group=PEnd, width=1)) +
  ...
  theme(plot.title=element_text(face="plain", colour="black", size=22, hjust=0.5)) +
  theme(axis.line=element_line(colour="black", linetype="solid", size=0.5)) +
  scale_fill_gradient2(name=expression(atop('Predator density', paste('('*rotifers~mL^-1*')'))), 
...
Plot1ODE2[Plot1ODE2$Organism=="A1",]$Density=log(MeltODE2[MeltODE2$Organism=="A1",]$Density+1)/5
Plot1ODE2[Plot1ODE2$Organism=="A2",]$Density=log(MeltODE2[MeltODE2$Organism=="A2",]$Density+1)/5
...
# Represent the population dynamics
tiff('Panel Population Dynamics Evolution 1.tiff', units="in", width=20, height=20, res=1000)
ggplot(Plot1ODE2, aes(Time, Density, group=Organism)) +
  ...
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank()) +
  theme(axis.line=element_line(colour="black", linetype="solid", size=0.5)) +
  scale_color_manual(values=c("A1"="royalblue2","A2"="cornflowerblue","P"="firebrick3")) +
  theme(strip.background=element_blank(), strip.text.x=element_blank()) +
  annotate("segment", x=-Inf, xend=Inf, y=-Inf, yend=-Inf, size=1.2) +
  annotate("segment", x=-Inf, xend=-Inf, y=-Inf, yend=Inf, size=1.2) +
  annotate("segment", x=Inf, xend=Inf, y=-Inf, yend=Inf, size=1.4) +
...
tiff('Panel Population Dynamics Evolution 2.tiff', units="in", width=20, height=20, res=1000)
...
# Calculate the difference in ingestion probabilities
SaveODE2$P0=as.character(1-as.numeric(SaveODE2$P1))
...
ggplot(subset(Data6, Organism=="1P"), aes(B, P0, width=1)) +
  ...
  colours=c("firebrick3","white","royalblue2"), values=rescale(x=c(0,6.4,8),from=c(0,8)), na.value="white", breaks=c(-28,6), limits=c(-30,8)) +
  scale_pattern_manual(values=c("EX"="circle","CRC"="none","EC"="none","SS"="none")) +
  theme(strip.background=element_blank(), strip.text.x=element_blank()) +
...
Data7=rbind(TimeODE1A[,c(3,7:8,1:2,6)],TimeODE2A[,c(3,9:10,1:2,8)])
...
Data7$P1=as.character(Data7$P1); Data7$B=as.character(Data7$B)
...
ggplot(Data7, aes(B, P0, width=1)) +
  ...
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank()) +
  theme(axis.line=element_line(colour="black", linetype="solid", size=0.5)) +
  scale_fill_gradient2(name=expression('Predator density'~'('*rotifers~mL^-1*')'),
...
###############################################
### Eco-evolutionary rescue or facilitation ###
###############################################
...
ggplot(subset(Data6, Organism=="2P"), aes(B, P0, width=1)) +
  ...
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank()) +
  theme(axis.line=element_line(colour="black", linetype="solid", size=0.5)) +
  scale_fill_manual(values=alpha(c("IEP"="firebrick3","IER"="royalblue2","IEF"="forestgreen","No"="grey80"),0.7)) +
  scale_pattern_manual(values=c("EX"="circle","EC"="none","CRC"="none","EC"="none","SS"="none")) +
  theme(plot.title=element_text(face="plain", colour="black", size=22, vjust=2, hjust=0.5)) +
  annotate("segment", x=-Inf, xend=-Inf, y=-Inf, yend=Inf, size=1.2) +
  annotate("segment", x=-Inf, xend=Inf, y=-Inf, yend=-Inf, size=1.2) +
...
ExtraODE1=subset(SaveODE1, Time==99 & Phase=="Pre")
ExtraODE2=subset(SaveODE2, Time==99 & Phase=="Pre")
ExtraODE1=cbind(ExtraODE1[,c(1:3)], A2=rep(0,209), ExtraODE1[,c(4:8)])
ExtraODE2=cbind(ExtraODE2[,c(1:4)], ExtraODE2[,c(6:10)])

# Replace zero values
ExtraODE1$A1[ExtraODE1$A1 == 0]=0.2
ExtraODE1$P[ExtraODE1$P == 0]=1.0
ExtraODE2$A1[ExtraODE2$A1 == 0]=0.1
ExtraODE2$A2[ExtraODE2$A2 == 0]=0.1
ExtraODE2$P[ExtraODE2$P == 0]=1.0

# Bind the datasets
ExtraODE=rbind(ExtraODE1,ExtraODE2)
ExtraODE$Organism=c(rep("1P",209),rep("2P",209))

# Extract equilibrium densities
ExtraODE[,c(2:5)]=round(ExtraODE[,c(2:5)],4)
write.table(ExtraODE, file="~/Activité Professionnelle/LIMNO 2019-2022/Modeling/Data_ED.txt", sep="\t", row.names=F)
...
# Create the datasets
Data8=subset(SaveODE1, Time %in% c(0:200) & P1 %in% c("0.1","0.6","0.9") & B %in% c("0.02","0.03","0.04"))
Data9=subset(SaveODE2, Time %in% c(0:200) & P1 %in% c("0.1","0.6","0.9") & B %in% c("0.02","0.03","0.04"))

# Rescale densities
Data8$A1=Data8$A1*10^6
Data9$A1=Data9$A1*10^6
Data9$A2=Data9$A2*10^6

# Create a dataset
Data8=data.frame(Data8[,c(1:3)], A2=0, Data8[,c(4:10)])
Data9=data.frame(Data9[,c(1:4)],Data9[,c(6:12)])
Data10=rbind(Data8[,c(1:11)],Data9[,c(1:11)])
Data10$System=c(rep("1P",1809),rep("2P",1809))
...
Data10$De=c(Data10[1:1809,]$P1,(Data10[1810:3618,]$FA1*Data10[1810:3618,]$P1+Data10[1810:3618,]$FA2*1.0))*50

# Melt the datasets
Data10=melt(Data10[,-c(2,13:14)], id.vars=c("Time","Ni","D","P1","P0","B","De","Phase","System"), variable.name="Organism", value.name="Density")
...
tiff('Population Dynamics No Evolution.tiff', units="in", width=20, height=20, res=1000)
ggplot(subset(Data10, System=="1P" &!Organism=="A"), aes(Time, Density)) +
  ...
  sec.axis=sec_axis(~./5*15, expression('Ln prey density'~'('*cells~mL^-1*')'), labels=function(y) sprintf("%.0f", y), breaks=seq(0,15,by=3))) +
...
tiff('Population Dynamics Evolution.tiff', units="in", width=20, height=20, res=1000)
...
ggplot(subset(Data11, System=="1P"), aes(P, A)) +
  ...
  ylab(expression(italic('Ln C. reinhardtii')~'density'~'('*cells~mL^-1*')')) + 
  xlab(expression(italic('Ln B. calyciflorus')~'density'~'('*rotifers~mL^-1*')')) +  
  theme(axis.text.y=element_text(face="plain", colour="black", size=24)) +  
  theme(axis.text.x=element_text(face="plain", colour="black", size=24)) + 
  theme(axis.title.y=element_text(face="plain", colour="black", size=24)) +
  theme(axis.title.x=element_text(face="plain", colour="black", size=24)) +
