...
OutHIII=lapply(SplitData2[-c(13,14,17:24)], FuncHIII)

RateHIII=bind_rows(lapply(OutHIII, function (x) x[c("Rates")]))
RateHIII=round(as.data.frame(do.call("rbind",RateHIII)),4)
RateHIII=cbind(Strain=Strain[-c(73:78,79:84,97:144)],Bead=Bead[-c(73:78,79:84,97:144)],Model="Holling III",RateHIII)
rownames(RateHIII)=c()
...
DataHIII=data.frame(Data3[-c(1801:1950,1951:2100,2401:3600),], IngesP=unlist(IngesP), IngesPLSD=unlist(IngesP)-unlist(IngesPSD), IngesPUSD=unlist(IngesP)+unlist(IngesPSD), IngesPLCI=unlist(IngesPLCI), IngesPUCI=unlist(IngesPUCI), Model="Holling III")
...
Strain1=Strain[-c(13,14,17:24)]
Bead1=Bead[-c(13,14,17:24)]
...
for (i in 1:length(Strain)) {
  for (j in 1:length(Strain1)) {
    ...
    LR2[[j]]=lrtest(ModelHIII[[j]],ModelHI[-c(13,14,17:24)][[j]])
  }
}
